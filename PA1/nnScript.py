...
def preprocess():
    ...
    #print(all_data_reduced.shape)
    #print(all_data_reduced)
...
def nnObjFunction(params, *args):
    ...
    # Your code here
    num_of_examples = training_data.shape[0]
    
    # Add bias to training data
    bias_for_input = np.ones(num_of_examples)
    new_train = np.c_[training_data, bias_for_input]
    # Hidden layer output
    hidden_output = sigmoid(np.dot(new_train, w1.transpose()))
    
    #Add bias to hidden_output
    new_hidden_output = np.c_[hidden_output, bias_for_input]
    
    # Output layer output
    output =sigmoid(np.dot(new_hidden_output, w2.transpose()))
    ...
    for i in range(num_of_examples) :
        ...
        true_label = int(training_label[i])
        # Decode y
        y = np.zeros(n_class)
        y[true_label] = 1.0
        
        #Calculate error function value for one example
        obj_val += np.sum(y * np.log(output[i]) + (1.0 - y) * np.log(1.0-output[i]))
        ...
        xp = training_data[i]
        #Add bias to example
        xp = np.hstack((xp, np.ones(1)))
        new_zj = np.hstack((zj, np.ones(1)))
        
        #Get gradient_W2
        grad_w2 += np.dot(delta.reshape(n_class,1), new_zj.reshape(1, n_hidden + 1))
        
        #Get gradient_W1
        zj = (1.0 - zj) * zj
        delta_multi = delta.dot(w2)[0:n_hidden].reshape(n_hidden, 1)
        grad_w1 += np.dot(zj.reshape(n_hidden, 1), xp.reshape(1, n_input + 1)) * delta_multi  
    
    # Add regularization
    obj_val = - obj_val / float(num_of_examples) + lambdaval / (2 * num_of_examples) * (np.sum(w1 ** 2) + np.sum(w2 ** 2))

    #Add regularization
    grad_w1 = 1.0 / num_of_examples * (grad_w1 + lambdaval * w1)
    grad_w2 = 1.0 / num_of_examples * (grad_w2 + lambdaval * w2)           
...
def nnPredict(w1, w2, data):
    ...
    # Add bias to training data
    bias_for_input = np.ones(num_of_examples)
    new_data = np.c_[data, bias_for_input]
    # Hidden layer output
    hidden_output = sigmoid(np.dot(new_data, w1.transpose()))
...
obj_selected = [selected_features, n_hidden, w1, w2, lambdaval]
# selected_features is a list of feature indices that you use after removing unwanted features in feature selection step
pickle.dump(obj_selected, open('/Users/chenshihchia/PycharmProjects/MLassignment1/params.pickle', 'wb'))
